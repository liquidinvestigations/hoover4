...
#   * INGEST_ROOT_FILENAMES is `qa/filenames` in the testdata checkout: one
#     document whose only search match is its filename. The body is digits so
#     no word of it can collide with a word of the name.
...
#   INGEST_ROOT_EMAILS    disk root for other/emails (bytes from testdata_emails)
#   INGEST_ROOT_ZIPS      disk root for testdata/zips
#   INGEST_ROOT_SHAPES    disk root for testdata/shapes (deep + wide VFS shapes)
#   INGEST_ROOT_FILENAMES disk root for testdata/filenames
# Unset roots are filled from /testdata/hoover-testdata/qa/datasets.json. An
# already-set variable, including the empty string, is left alone.
set -euo pipefail
...
TESTDATA_MOUNT="/testdata/hoover-testdata"
DATASETS_JSON="${DATASETS_JSON:-$TESTDATA_MOUNT/qa/datasets.json}"
# INGEST_ROOT_* defaults are filled from DATASETS_JSON after the worker is up.
# An already-set variable, including the empty string, is left alone so a caller
# can still switch a root off with `INGEST_ROOT_SHAPES=`.
...
import json, sys
name, mount, path = sys.argv[1], sys.argv[2], sys.argv[3]
data = json.load(open(path))
for d in data["datasets"]:
    if d["name"] == name:
        print(mount + "/" + d["root"])
        raise SystemExit(0)
raise SystemExit("dataset %s not in %s" % (name, path))
' "$name" "$TESTDATA_MOUNT" "$DATASETS_JSON"
}

# Fill an unset ingest root from datasets.json. A variable that is set, even to
# empty, is left alone so a caller can still switch a root off.
fill_ingest_root() {
    local var="$1" dataset_name="$2"
    if [ -n "${!var+x}" ]; then
        return 0
    fi
    local root
    root=$(dataset_root "$dataset_name") || return 1
    printf -v "$var" '%s' "$root"
}

load_ingest_roots() {
    fill_ingest_root INGEST_ROOT_TESTDATA testdata_testfiles
    fill_ingest_root INGEST_ROOT_EMAILS testdata_emails
    fill_ingest_root INGEST_ROOT_ZIPS testdata_zips
    fill_ingest_root INGEST_ROOT_SHAPES testdata_shapes
    fill_ingest_root INGEST_ROOT_FILENAMES testdata_filenames
    echo "     testdata <- ${INGEST_ROOT_TESTDATA-<unset>}"
    echo "     emails   <- ${INGEST_ROOT_EMAILS-<unset>}"
    echo "     zips     <- ${INGEST_ROOT_ZIPS-<unset>}"
    echo "     shapes   <- ${INGEST_ROOT_SHAPES-<unset>}"
    echo "     filenames<- ${INGEST_ROOT_FILENAMES-<unset>}"
}
...
echo "== ingest roots from $DATASETS_JSON =="
load_ingest_roots
...
# Do not write it at run time: a generated file cannot give parity across hosts.
require_filename_hit_fixture() {
    if [ -z "${INGEST_ROOT_FILENAMES:-}" ]; then return 0; fi
    local fixture="$INGEST_ROOT_FILENAMES/${FILENAME_HIT_TOKEN}.txt"
    if docker exec "$WORKER" test -f "$fixture"; then
        return 0
    fi
...
if [ -z "$api_path" ]; then
    ...
    # The filename-only fixture's whole point: its name matches and its body cannot, so the
