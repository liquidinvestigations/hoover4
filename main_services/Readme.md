...
- `bench-overhead.py` — per-probe p50/mean/min/max for ClickHouse client/insert, Magika, extractous and `file`. Runs inside `hoover4-worker`: `docker exec -i hoover4-worker uv run python - < main_services/bench-overhead.py`.
