...
///
/// The digits are `[0-9]`, not `\d`. Unicode `\d` is dozens of ranges, and twenty-nine prefixes
/// each followed by overlapping counted runs of it are, of every pattern in the set, the one that
/// drives the combined prefilter automaton past its DFA cache.
const VAT_EU_PATTERN: &str = concat!(
    ...
    r"|HR[0-9]{11}",
    r"|HU[0-9]{8}",
    r"|IE[0-9][0-9A-Z+*][0-9]{5}[A-Z]{1,2}",
    r"|IT[0-9]{11}",
    r"|LT[0-9]{12}|LT[0-9]{9}",
    r"|LU[0-9]{8}",
    r"|LV[0-9]{11}",
    r"|MT[0-9]{8}",
    r"|NL[0-9]{9}B[0-9]{2}",
