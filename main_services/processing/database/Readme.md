...
| `00037_operation_failures.sql` | Global. One row per node of an operation failure tree, keyed `(op_id, node_index)` so one operation's tree is a prefix read. Monthly partition on `captured_at` so a newest-failures query reads recent partitions. TTL 180 days. No pipeline stage writes to this table. |
