...
(
    ...
    -- nlp_model is in the key from the start: two NER providers run over every text
    -- variant, and both sets coexist by design. Leave it out of ORDER BY and the second
    -- provider silently replaces the first rather than adding to it.
    nlp_model LowCardinality(String) DEFAULT '' COMMENT 'NER provider that produced the hit: ner-gpu-xlmr | ner-spacy-xx',
...
ORDER BY (collection_dataset, file_hash, extracted_by, page_id, nlp_model, entity_type)
