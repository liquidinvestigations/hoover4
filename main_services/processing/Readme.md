...
- Retry historical Error rows with `python main.py retry-failed-files <collectionname> [--dataset X] [--task P4_ExtractEntities] [--apply]`. Apply requires `--task`. The operation selects matching Error rows, clears their stage state and reopens their plans. It removes an older Error only after an exact successful task run and document outcome, or a current Error for that pair. Skipped inputs have no `processing_errors` row and are outside this command.
