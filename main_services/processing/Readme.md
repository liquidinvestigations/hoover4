...
- Retry historical Error rows with `python main.py retry-failed-files <collectionname> [--dataset X] [--task P4_ExtractEntities] [--apply]`. The operation selects matching Error rows, clears their stage state, reopens their plans, and reconciles the Error rows after execution. Skipped inputs have no `processing_errors` row and are outside this command.
