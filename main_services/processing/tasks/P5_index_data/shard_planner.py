...
def plan_shards(params: PlanShardsParams) -> list[ShardAssignment]:
    ...
    log.info(
        "[P5] plan_shards %s (plan %s): %d new docs, %d re-assigned, %d shards (%d new)",
...
def finalize_index_batch(params: FinalizeIndexBatchParams) -> str:
    ...
    log.info(
        "[P5] finalize_index_batch %s (plan %s): ledger refreshed",
...
def record_indexed(params: RecordIndexedParams) -> str:
    ...
    log.info(
        "[P5] record_indexed %s (plan %s): %d documents recorded",
