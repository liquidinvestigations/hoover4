...
never the configured `embeddings_model` from the ini. Note for anyone reverting to
...
def embedding_input(model_id: str, kind: str, text: str) -> tuple[str, str | None]:
    ...
    `kind` is `"passage"` (a stored chunk — what this stage embeds) or `"query"` (a
    search query — the collection-search server's half of the convention). Returns
