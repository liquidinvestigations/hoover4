...
7. **Correction package.** Only on `reject`. A complete work package in the form of
   ...
   blocking finding and no non-blocking finding.

**A review of a correction stays inside that correction's diff.** Read the diff and the findings
the correction was sent to close. Report whether each finding is closed and whether the diff is
correct. A defect you see outside that diff is a non-blocking finding with the action `observe`,
and it cannot make your verdict `reject`.
...
plan, because the organizer ends a batch on a second occurrence, and acts on a third occurrence
in the plan.
