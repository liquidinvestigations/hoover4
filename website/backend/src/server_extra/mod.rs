...
use crate::api::documents::download_document::{get_blob_filename, get_document_blob_content_from_clickhouse, get_document_download_path};
...
async fn _download_document(Path((collection_dataset, file_hash)): Path<(String, String)>) -> anyhow::Result<impl IntoResponse> {
    ...
    let blob_info = get_document_download_path(document_identifier.clone()).await?;
    ...
    if blob_info.stored_in_clickhouse {
        ...
        let s3_endpoint = "http://10.69.70.112:9004";
