...
use common::{
    ...
    pdf_to_html_conversion::PDFToHtmlConversionResponse, search_result::DocumentIdentifier,
};
use reqwest::Body;
use serde::{Deserialize, Serialize};

use crate::api::documents::{
    download_document::get_document_content_stream, get_raw_metadata::get_raw_metadata,
};
...
pub async fn get_pdf_to_html_single_page(
    document_identifier: DocumentIdentifier,
    page_index: u32,
) -> anyhow::Result<PDFToHtmlConversionResponse> {
    let mut doc = get_pdf_to_html_conversion(document_identifier).await?;
    doc.pages = vec![doc.pages[page_index as usize].clone()];
    Ok(doc)
}

pub(crate) async fn get_pdf_to_html_conversion(
    document_identifier: DocumentIdentifier,
) -> anyhow::Result<PDFToHtmlConversionResponse> {
    ...
    let mut insert = client
        .insert::<PDFToHtmlCacheRow>("pdf_to_html_cache")
        .await?;
...
async fn _make_pdf_to_html_conversion(
    document_identifier: DocumentIdentifier,
) -> anyhow::Result<PDFToHtmlConversionResponse> {
    ...
    let (stream_size, doc_stream) =
        get_document_content_stream(document_identifier.clone()).await?;
    tracing::info!("Document stream received");
    let client = reqwest::Client::new();
    let response = client
        .post(format!(
            "{}",
            std::env::var("PDF_TO_HTML_ENDPOINT").context("PDF_TO_HTML_ENDPOINT is not set")?
        ))
    ...
    assert!(
        _page_count == body.pages.len() as u32,
        "page count mismatch"
    );
    Ok(body)
}
