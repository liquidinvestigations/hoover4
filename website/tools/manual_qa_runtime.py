...
async def tree(r):
    ...
    async def two_datasets():
        ...
        await r.action("history_back")
        return await r.check("const keys=[...document.querySelectorAll('#x-storage-tree [data-node-key]')].map(e=>e.dataset.nodeKey);return {ok:keys.some(x=>x.startsWith('testdata_shapes'))&&keys.some(x=>x.startsWith('testdata_zips')),visibleNodeKeys:keys};")
    ...
    async def retained():
        ...
        await r.action("eval", "window.__qa_deep_route=location.pathname;window.__qa_deep_rows=[...document.querySelectorAll('#x-storage-tree [data-node-key]')];const current=window.__qa_deep_rows.find(x=>x.getAttribute('aria-current')==='location');window.__qa_deep_ancestors=window.__qa_deep_rows.filter(x=>current?.title.startsWith(x.title+'/'));window.__qa_deep_removed=[];window.__qa_deep_observer=new MutationObserver(records=>{for(const rec of records)for(const n of rec.removedNodes)for(const row of window.__qa_deep_ancestors)if(n===row||n.contains?.(row))window.__qa_deep_removed.push(row.title)});window.__qa_deep_observer.observe(document.querySelector('#x-storage-tree'),{childList:true,subtree:true});window.__qa_deep_requests=performance.getEntriesByType('resource').filter(x=>x.name.includes('/api/vfs_tree_')).length;return window.__qa_deep_rows.map(x=>({key:x.dataset.nodeKey,title:x.title}));")
        await r.action("eval", "const rows=[...document.querySelectorAll('#x-storage-tree [data-node-key]')];const current=rows.find(x=>x.getAttribute('aria-current')==='location');const parent=rows.filter(x=>current?.title.startsWith(x.title+'/')).at(-1);if(!parent)throw Error('deep parent row unavailable');window.__qa_deep_current_title=current.title;window.__qa_deep_parent_title=parent.title;parent.click();return true;")
        ...
        await r.check("const rows=[...document.querySelectorAll('#x-storage-tree [data-node-key]')];return rows.some(x=>x.title===window.__qa_deep_parent_title)&&!rows.some(x=>x.title.startsWith(window.__qa_deep_current_title+'/'));")
        await r.action("history_back")
        await r.check("return location.pathname===window.__qa_deep_route;")
        await r.check("return document.querySelector('#x-storage-tree [aria-current=location]')?.title===window.__qa_deep_current_title;")
        await r.action("async_eval", "await new Promise(requestAnimationFrame);await new Promise(requestAnimationFrame);return true;")
        return await r.action("eval", "const common=window.__qa_deep_ancestors;const after=performance.getEntriesByType('resource').filter(x=>x.name.includes('/api/vfs_tree_')).length;const result={ancestors:common.map(x=>x.title),removed:window.__qa_deep_removed,requestsBefore:window.__qa_deep_requests,requestsAfter:after};if(!common.length||common.some(x=>!x.isConnected)||window.__qa_deep_removed.length)throw Error('deep ancestor DOM replaced: '+JSON.stringify(result));return result;")
