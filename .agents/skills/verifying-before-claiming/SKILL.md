...
- A green `cargo check` standing in for a frontend claim — it misses server-build and hook
  errors that only `dx check` and a real page load surface.
...
they are never built, and a broken integration-test binary can sit unnoticed for a long time.
