...
| `F-chat-18a` | Delegate exactly one level deep, because a worker's tool list is built without the delegation tool rather than because its instructions ask it not to recurse | `subagents.worker_allowed_tools`, `subagents.IN_PROCESS_WORKER_EXCLUDED` |
...
| `F-chat-18c` | Give a worker the `subagent` tool packs less the tools that drive a page and the plan writers, so a delegating turn costs the browser server what a plain turn costs it and no worker rewrites the conversation's plan | `subagents.IN_PROCESS_WORKER_EXCLUDED` |
