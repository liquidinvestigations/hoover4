...
| `F-auth-08` | Resolve a caller of `/api/agent/v1/` from `X-Hoover4-User`, refuse a request carrying a proxy identity header or a session cookie, and expose the site's read surface to a tool-calling agent as eighteen typed routes over collections, search, documents, tables and folders | `website/backend/src/auth/session_middleware.rs`, `website/backend/src/api/agent/`, `website/common/src/agent_api.rs` |
