...
| `F-ingest-12` | Re-run selected Error rows through their affected plans, without re-ingesting, and reconcile each Error row after execution | `main.py retry-failed-files`, the `retry_failed_files` operation |
...
| `F-admin-11` | Record every long operation permanently (what was asked for, by whom, its progress, and how it ended), its Error events and the plans it ran, outliving both the process that asked and the workflow history | the `operations`, `operation_error_events` and `operation_plans` tables, `main_services/processing/database/operations.py` |
...
| `F-admin-15` | List, inspect, re-run and cancel operations from the command line, projecting a re-run to its accepted inputs before it writes a row | `main.py operations list\|show\|rerun\|cancel`, `database/operation_inputs.py` |
| `F-admin-16` | Browse the operations log in the interface (newest first, paginated, filtered by state and by collection) with progress, estimate, outcome, a re-run summary line, a Temporal deep link on every row, and a link to the captured failure tree when one exists | `/admin/operations`, `website/backend/src/api/admin/operations.rs` |
| `F-admin-17` | Re-run or cancel an operation from the interface, where a destructive kind is refused until the target is typed out and a re-run projects accepted inputs before it writes a row | `admin_rerun_operation`, `admin_cancel_operation` |
...
| `F-admin-19` | Show the run's failed documents and failed tasks. Show recovered, still-failing, disabled-stage, unmappable and historical Error-pair counts separately. | `tasks/P_ops/activities.py:sample_dataset_progress`, `tasks/P_admin/rerun_selection.py`, the row's `detail` |
