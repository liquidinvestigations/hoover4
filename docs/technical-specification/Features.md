...
| `F-chat-22a` | Return each collection MCP data read as one canonical result page of whole units, and continue through `read_more` with the route's own next position. Store a backend window that does not fit one page once, as a required `detail.json` artifact, and read its later pages as owner-checked byte ranges of it. Store a unit larger than a page with string fields moved out, largest first, until the rest fits a page. Cut its page inside the first moved field, with a marker that lists the other moved fields, and continue through the rest of each moved field in order and then the next unit. Refuse a stored unit that stays larger than a page with no string field left to move, with `invalid_argument`. | `collection_search_server/paging.py`, `agent_common/result_pages.py`, `agent_common/artifacts.py` |
