...
* **right** — **Entities** and **Metadata** tabs.
...
**File locations** matters more than it looks: the same content can sit at several paths, and
...
Server settings you can change without a redeploy — chat artifact retention, guest access mode,
the default chat and summarisation models, and the session lifetime. Below them, deployment
...
* a document has a permanent link — *Copy Document Link* in the ⋮ menu;
